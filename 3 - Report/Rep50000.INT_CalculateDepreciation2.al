report 50000 "INT_Calculate Depreciation 2"
{
    ...
    dataset
    {
        ...
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            begin
                ...
                //AVPKJINT.001 08/11/2019
                DaysInPeriod := DaysInPeriod;

                Clear(FADeprBook);
                FADeprBook.SetCurrentKey("FA No.", "Depreciation Book Code");
                ...
                if FADeprBook.Find('-') then begin
                    repeat
                        ...
                        if FADeprBook.Depreciation = 0 then begin
                            //1. Case Skip : Depreciation = 0 And Depre Start Date = 0D 
                            if FADeprBook."Depreciation Starting Date" = 0D then begin
                                //CurrReport.Skip;
                            end else begin
                                //2. Case : Depreciation = 0 And Depre Start Date <> 0D 
                                if FADeprBook."Last Depreciation Date" = 0D then
                                    DaysInPeriod := DeprUntilDate - FADeprBook."Depreciation Starting Date" + 1;
                            end;
                        end else begin
                            //3. Case : Depreciation <> 0
                            //DaysInPeriod := DATE2DMY(DeprUntilDate, 1) - DATE2DMY(FADeprBook."Last Depreciation Date", 1);
                            DaysInPeriod := DeprUntilDate - FADeprBook."Last Depreciation Date";
                        end;
                    until FADeprBook.Next = 0;
                end;
                //C-AVPKJINT.001 08/11/2019
    ...
    var
        ...
        DeprBookCode: Code[10];     //AVPKJINT.001 08/11/2019 
        BalAccount: Boolean;
        ErrorNo: Integer;
