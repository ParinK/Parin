report 50005 "INT_Trial Balance by Dep"
{
    UsageCategory = Administration;
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = 'Rdlc/Trial Balance by Department.rdl';
    Caption = 'Trial Balance by Department';
        ...
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            */
            ...
            dataitem(Integer; Integer)
            {
                ...
                begin
                    ...
                    (COPYSTR("G/L Account"."No.", 1, 1) = '6') OR (COPYSTR("G/L Account"."No.", 1, 1) = '7') THEN BEGIN
                        ...
                        DimValue_Dep.SETCURRENTKEY(DimValue_Dep."Dimension Code");
                        DimValue_Dep.SETRANGE(DimValue_Dep."Dimension Code", 'COST CENTER');
                        IF "G/L Account".GETFILTER("G/L Account"."Global Dimension 1 Filter") <> '' THEN
                            DimValue_Dep.SETFILTER(DimValue_Dep.Code, TxtFilter);
                        IF DimValue_Dep.FINDFIRST THEN
                            CountDep := DimValue_Dep.COUNT;
                    END;
                ...
                begin
                    ...
                    ELSE
                        IF CountAccount = 2 THEN
                            FilterDepCode := DimValue_Dep.Code
                        ELSE
                    ...
                    IF CountAccount = 1 THEN
                        FilterDepCode := DimValue_Dep.Code
                    ELSE IF CountAccount > 1 THEN BEGIN
                        DimValue_Dep.NEXT;
                        FilterDepCode := DimValue_Dep.Code;
                    END;
                    END;*/
            ...
            begin
                ...
                //AVWNFCNX.001 270919
                IF CostOption = CostOption::DL THEN BEGIN
                    "G/L Account".SETFILTER("G/L Account"."No.", '500*|501*|510*|511*|512*');
                    "G/L Account".SETRANGE("G/L Account"."Global Dimension 1 Filter", '2751-0*');
                END ELSE
                    IF CostOption = CostOption::OH THEN BEGIN
                        "G/L Account".SETFILTER("G/L Account"."No.", '5*&<>500*&<>501*&<>510*&<>511*&<>512*&<>579*');
                        "G/L Account".SETRANGE("G/L Account"."Global Dimension 1 Filter", '2751-0*');
                    END ELSE
                        IF CostOption = CostOption::"I-OH" THEN BEGIN
                            "G/L Account".SETFILTER("G/L Account"."No.", '5*&<>5731*&<>579*');
                            "G/L Account".SETRANGE("G/L Account"."Global Dimension 1 Filter", '2751-1*');
                        END;
                //C-AVWNFCNX.001 270919
            end;
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    */
                    field("Hide Zero"; ShowZero)
                    {
                        ApplicationArea = All;
                    }
                    field("Option"; CostOption)
                    {
                        ApplicationArea = All;
                    }
                }
    ...
    var
        ...
        DimValue_Dep: Record "Dimension Value";
        GLFilter: Text;
        PeriodText: Text[30];
